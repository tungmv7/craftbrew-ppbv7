...
class Register extends AbstractBaseForm
{
    ...
    public function __construct($formId = null, $action = null, $user = null, $displaySubtitles = true)
    {
        ...
        $this->addElements($this->_model->getElements());

        // check data from facebook login
        $session = Front::getInstance()->getBootstrap()->getResource('session');
        if ($fbData = $session->get('facebookUserProfile')) {
            if (isset($this->_elements['email'])) {
                $this->_elements['email']->setValue($fbData['email']);
            }
        }
